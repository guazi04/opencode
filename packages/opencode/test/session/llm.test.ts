...
describe("session.llm.stream", () => {
  test("does not duplicate agent prompt when user system matches it", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const sessionID = SessionID.make("session-test-system-dedupe")
        const text = "Main session system prompt"
        const agent = {
          name: "test",
          mode: "primary",
          prompt: text,
        ...
        const user = {
          id: MessageID.make("user-system-dedupe"),
          sessionID,
          role: "user",
        ...
        const rows = (capture.body.messages ?? []) as Array<{ role?: string; content?: unknown }>
        const joined = rows
          .filter((x) => x.role === "system")
          .map((x) => (typeof x.content === "string" ? x.content : JSON.stringify(x.content)))
          .join("\n")
        const count = joined.split(text).length - 1
        expect(count).toBe(1)
      },
    })
  })
